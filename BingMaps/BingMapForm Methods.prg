...
METHOD FillLocations(locs AS LocationCollection) AS VOID
	...
	cStatement:="SELECT DateTimeGMT, Latitude, Longitude, N_OR_S, W_OR_E, GmtDiff"+;
				...
				" AND  FMDataPackages.Visible=1 AND FMDataPackages.REPORT_UID="+SELF:cReportUID+;
...
METHOD CreatePushpin() AS VOID
	...
	LOCAL p := SELF:MyMapUserControl:AddPushpin(SELF:MyMapUserControl:PinLayer, SELF:MyMapUserControl:currentPushpinLocation, cTitle, cInfo, FALSE, NULL, NULL) AS Pushpin
...
METHOD LocationInfo(oPosition AS Location, dDate AS DateTime, nGmtDiff AS Decimal) AS STRING
	...
	cStatement:="SELECT FMReportItems.ItemName, FMData.Data"+;
				...
				" AND FMDataPackages.REPORT_UID="+SELF:cReportUID+" AND FMDataPackages.Visible=1 "+;
